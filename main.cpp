...
extern void fc_layer(size_t data_cnt, size_t input_dim, size_t output_dim, float *matrix, float *bias, float *input, float *output);
...
int main(int argc, char **argv)
{
	...
	if (argc < 3)
	{
		...
		fprintf(stderr, "Example: %s data/input.32.bin data/output.32.bin\n", argv[0]);
		exit(10);
	}
	...
	while (true)
	{
		...
		if (input_words != model_dim_in || output_words != model_dim_out)
			break;
		input_cnt++;
	}
	if (input_cnt < 1)
	...
	fc_layer(input_cnt, model_dim_in, model_dim_out, model_matrix, model_bias, inputs, outputs);
